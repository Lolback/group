...
public class TestListSubjectDao extends Dao{
	...
	public List<TestListSubject> filter(Student student, School school) throws Exception{
		...
		try {
			//プリペアードステートメントにSQL文をセット
			statement = connection.prepareStatement("SELECT DISTINCT result1.ENT_YEAR, result1.CLASS_NUM, result1.NO, result1.NAME, result1.SUBJECT_NAME, NO1, POINT1, NO2, POINT2 FROM (SELECT s.ENT_YEAR, s.CLASS_NUM, s.NO, s.NAME, sub.NAME AS SUBJECT_NAME, t.NO AS NO1, COALESCE(t.POINT, 0) AS POINT1 FROM STUDENT s JOIN SUBJECT sub ON sub.SCHOOL_CD = s.SCHOOL_CD LEFT JOIN TEST t ON s.NO = t.STUDENT_NO AND sub.CD = t.SUBJECT_CD WHERE s.SCHOOL_CD = ? AND s.NO = ? AND t.NO = 1) as result1 JOIN (SELECT s.ENT_YEAR, s.CLASS_NUM, s.NO, s.NAME, sub.NAME AS SUBJECT_NAME, t.NO AS NO2, COALESCE(t.POINT, 0) AS POINT2 FROM STUDENT s JOIN SUBJECT sub ON sub.SCHOOL_CD = s.SCHOOL_CD LEFT JOIN TEST t ON s.NO = t.STUDENT_NO AND sub.CD = t.SUBJECT_CD WHERE s.SCHOOL_CD = ? AND s.NO = ? AND t.NO = 2) as result2 ON result1.NO = result2.NO");
			//ステートメントにデータをバインド
			statement.setString(1, school.getCd());
